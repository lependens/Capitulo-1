...
def process_station_data(code, estaciones_df):
    station_file = os.path.join(data_path, f'{code}_datos_depurados.csv')  # Usa depurados si prefieres
    ...
    df['ET0_sun'] = np.nan
    df['diff_sun'] = np.nan
    df['Ra'] = np.nan  # Nueva columna para Ra
    ...
    for idx, row in df.iterrows():
        ...
        df.at[idx, 'Ra'] = Ra  # ¡Guardamos Ra aquí!
