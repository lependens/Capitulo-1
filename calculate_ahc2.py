...
def calculate_ahc_for_station(code):
    ...
    df = df.dropna(subset=['Fecha', 'EtPMon', 'ET0_calc', 'ET0_harg_ra', 'ET0_harg_rs', 'ET0_val'])  # Asegurar datos válidos
    ...
    df['A_diario_harg_ra'] = df['ET0_calc'] / df['ET0_harg_ra']
    df['A_diario_harg_rs'] = df['ET0_calc'] / df['ET0_harg_rs']
    df['A_diario_val'] = df['ET0_calc'] / df['ET0_val']
    
    # AHC como mean(ratios diarios), como en MATLAB
    ahc_harg_ra = np.nanmean(df['A_diario_harg_ra'])  # nanmean para manejar NaNs
    ahc_harg_rs = np.nanmean(df['A_diario_harg_rs'])
    ahc_val = np.nanmean(df['A_diario_val'])
    
    # ET0 ajustados (como en MATLAB: emp * ahc)
    df['ET0_harg_ra_ajustado'] = df['ET0_harg_ra'] * ahc_harg_ra
    df['ET0_harg_rs_ajustado'] = df['ET0_harg_rs'] * ahc_harg_rs
    df['ET0_val_ajustado'] = df['ET0_val'] * ahc_val
    
    # Guardar CSV actualizado
    output_file = os.path.join(data_path, f'{code}_et0_variants_ajustado.csv')
    df.to_csv(output_file, index=False)
    print(f"Procesado {code}: AHC_harg_ra = {ahc_harg_ra:.4f}, AHC_harg_rs = {ahc_harg_rs:.4f}, AHC_val = {ahc_val:.4f}. Guardado en {output_file}")
    
    return ahc_harg_ra, ahc_harg_rs, ahc_val

# Procesar todas y generar MD
def generate_ahc_md():
    ahc_data = {'Estacion': [], 'AHC_Hargreaves_Ra': [], 'AHC_Hargreaves_Rs': [], 'AHC_Valiantzas': []}
    ahc_harg_ra_list = []
    ahc_harg_rs_list = []
    ahc_val_list = []
    
    for code in estaciones:
        ahc_harg_ra, ahc_harg_rs, ahc_val = calculate_ahc_for_station(code)
        if ahc_harg_ra is not None:
            ahc_data['Estacion'].append(code)
            ahc_data['AHC_Hargreaves_Ra'].append(round(ahc_harg_ra, 4))
            ahc_data['AHC_Hargreaves_Rs'].append(round(ahc_harg_rs, 4))
            ahc_data['AHC_Valiantzas'].append(round(ahc_val, 4))
            ahc_harg_ra_list.append(ahc_harg_ra)
            ahc_harg_rs_list.append(ahc_harg_rs)
            ahc_val_list.append(ahc_val)
    
    # General (media de AHC por estación)
    if ahc_harg_ra_list:
        ahc_harg_ra_general = np.mean(ahc_harg_ra_list)
        ahc_harg_rs_general = np.mean(ahc_harg_rs_list)
        ahc_val_general = np.mean(ahc_val_list)
        ahc_data['Estacion'].append('General')
        ahc_data['AHC_Hargreaves_Ra'].append(round(ahc_harg_ra_general, 4))
        ahc_data['AHC_Hargreaves_Rs'].append(round(ahc_harg_rs_general, 4))
